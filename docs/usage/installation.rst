...
Install Rune and it's dependencies using ``pip``
------------------------------------------------
...
Set up the required boilerplate
-------------------------------
...
Create the config files ``.env``, ``.flaskenv``, and ``rune.conf``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Create the Admin / Dedicated User
---------------------------------
...
**IMPORTANT!**: On the first login you will be prompted to change it.
